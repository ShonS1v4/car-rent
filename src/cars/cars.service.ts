...
export class CarsService {
  ...
  public async setItem(data: CarDto): Promise<Cars> {
    try {
  ...
  public async rent(data: RentDto): Promise<object> {
    try {
      const car = await this.Cars.findOne({ id: data.id });
      if (car.is_available)
        return  await this.rent_success(data)
      else
        return {message: 'The car is currently busy'};
    } catch (e) {
        throw new HttpException(e, 400)
    }
  }
  ...
  //rent
  public async rent_success(data: any): Promise<object>{
    try {
      ...
      if (taken_from_day === 6 || taken_from_day === 0) {
        return {message: 'Impossible to rent a car on a weekend!'};
      }
      ...
      return {message: 'The maximum rental period is up to 30 days!'};
    } catch (e) {
      throw new HttpException(e, 400)
    }
  }

  //функция которая находит отчет об машине по ее id
  public async getOverviewByyCar(id: number) {
  ...
  public async getUtilByRow(data: KmDiscountDto) {
    try {
      if (data.type === types.TARIFF) {
        const { km } = data;
  ...
  public async deleteUtil(data: KmDiscountDto) {
    try {
      if (data.type === types.TARIFF) {
        const type = 'Tariffs';
        const id = data.id;
